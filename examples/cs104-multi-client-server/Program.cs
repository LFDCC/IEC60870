// This example shows a server that can handle several clients simultaneously.
// It tracks each client via the Iec104Session provided to the AsduReceived and
// ConnectionEvent callbacks, and replies to the correct session with
// session.SendAsync(...). Periodic data is broadcast to all activated sessions.
//
// NOTE: The old API offered a connection-request handler with an IP whitelist
// (SetConnectionRequestHandler). The new Iec104Server has no equivalent hook, so
// the IP-based access control is omitted here.
