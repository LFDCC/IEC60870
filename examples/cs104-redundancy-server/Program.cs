// This example shows a server handling multiple clients. The original example
// configured redundancy groups (ServerMode.MULTIPLE_REDUNDANCY_GROUPS,
// RedundancyGroup, AddAllowedClient, AddRedundancyGroup) to pair clients.
//
// NOTE: redundancy pairing is not part of the new API; this is a single server.
// Multiple clients are still supported: each is tracked via the Iec104Session
// provided to the AsduReceived/ConnectionEvent callbacks and answered with
// session.SendAsync(...). Periodic data is broadcast to all activated sessions.
