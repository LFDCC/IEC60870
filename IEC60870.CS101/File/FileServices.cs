 ...
 *  along with IEC60870.Core.NET.  If not, see <http://
 *  
...
namespace IEC60870.CS101.File
{
    ...
    public class FileClient
    {
        ...
        public FileClient(ClientBase master, DebugLogger debugLog)
        {
            this.master = master;
            DebugLog = debugLog;
        ...
        private bool SendSegment()
        {
            ...
            if (currentSegmentSize > 0)
            {
                ...
                lastSentTime = System.DateTimeOffset.UtcNow.ToUnixTimeMilliseconds();
            ...
            switch (asdu.TypeId)
            {
                ...
                case TypeID.F_FR_NA_1: /* File ready */
                    ...
                    else if (state == FileClientState.WAITING_FOR_REQUEST_FILE)
                    {
                        ...
                        if (asdu.IsNegative)
                        {
                            DebugLog("ServerBase rejected file download: " + asdu.Cot.ToString());
                        }
                        else
                        {
        ...
        public void HandleFileService()
        {
            ...
            if (state != FileClientState.IDLE)
            {
                if (System.DateTimeOffset.UtcNow.ToUnixTimeMilliseconds() > lastSentTime + timeout)
                {
